...
### Task 8: End-to-end check in the live herdr (Stoaf)

Rewritten 2026-09-18 after the final review. The original steps assumed a fresh `hashistack`
tab and a fresh `sysop` tab, but both lanes already have exact-label tabs, so those clicks
correctly FOCUS instead of creating. DECISIONS D6-D9 are already written (ruling R3), so
there is no decisions step here.

This task creates real tabs in Stoaf's working herdr session. **Stoaf runs it, or
explicitly OKs an agent doing it.** Every tab it creates can be closed afterwards.

**State this test runs in:** the session hooks are NOT installed on stoafmbp. That means:
live lanes are not hidden from the list; recorded tab links are not written by the hook;
and resume is disabled by design (LaunchPlan types plain `claude`, and the lane list shows
"Session hooks are off..."). Steps 1-5 test exactly that state. Step 6 is optional.
...
pkill -f ClaudeUsage.app; sleep 1
env -u CLAUDECODE -u CLAUDE_CODE_CHILD_SESSION -u CLAUDE_CODE_SESSION_ID \
  open ~/git/claudecodeusage/build/DerivedData/Build/Products/Debug/ClaudeUsage.app
```

Wait ~40s for the first board fetch, then open the popover.
Expected: a LANES section with rows (name, idle time, path or `no path`, `N open`), and
the hooks-off caption. No orange `lane data cached` label, and no `open counts
unavailable`.

- [ ] **Step 2: A lane that already has a tab focuses it**

Click `sysop`. Its row should show `~/git/agent-bbs` (from its lane card).
Expected: herdr switches to the EXISTING `sysop` tab. No new tab appears.
Check: `herdr tab list | grep -c '"label":"sysop"'` prints `1`.

- [ ] **Step 3: A no-path lane opens a plain shell**

Pick a row that says `no path` and has no tab with that exact name. Click it.
Expected: a new tab labelled with the lane, at `~`, showing a shell prompt. Claude does
NOT start. No red error under the list.

In that tab run:
```bash
echo "BBS_AGENT=$BBS_AGENT"; env | grep -c '^CLAUDE_CODE' || true
```
Expected: `BBS_AGENT=<the lane>`, then `0`. The zero proves the app's environment did not
leak into herdr tabs.

- [ ] **Step 4: A lane with a path and no tab starts a fresh claude**

Pick a row that shows a path and has no tab with that exact name (check first:
`herdr tab list | grep '"label":"<lane>"'` prints nothing). Click it.
Expected: a new tab labelled with the lane, in that path, with `claude` typed and
starting. Because hooks are off it is plain `claude`, NOT `--resume`, and the list shows
a red note starting "not resuming". If instead a red `New tab is running ..., not a shell`
error appears, note the process name it reports; that name belongs in the `shells` set in
`HerdrClient.runInShell`.

Then: `cat ~/.claude/claudeusage-lanes/lane-tabs.json` shows an entry for that lane with
the new tab id and cwd.

- [ ] **Step 5: Second click focuses, and a double-click does not duplicate**

Switch to any other herdr tab. Click the Step 4 lane again.
Expected: herdr focuses the Step 4 tab. `herdr tab list | grep -c '"label":"<lane>"'`
still prints `1`. Then double-click another fresh lane quickly: exactly one new tab.

- [ ] **Step 6 (optional, a separate decision): install the session hooks and test resume**

Install the hooks from the app's Settings window. This edits `~/.claude/settings.json`.
After that, the lanes you are working in drop out of the dormant list, and a dormant
lane with an old enough session resumes with `claude --resume <id>`. Whether to install
them is Stoaf's call. Until then, resume stays off on purpose.
